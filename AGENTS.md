...
## Changelog
- 2026-06-10 — Fix teks ambigu "Sisa kuota" pada popup Istura Open: sebelumnya merender rincian per-hari `${tanggal} ${remaining}` (mis. "Sisa kuota: 14 Jun 97" — tanggal & jumlah menempel, terbaca seperti tahun). Diganti jadi **satu total sisa kuota seluruh hari terbuka** dengan satuan eksplisit ("Sisa kuota: 97 orang"). Rincian per-hari (`Sisa {remaining}/{quota}`) tetap ada di langkah pemilihan hari wizard, jadi tidak ada info hilang. Popup hanya teaser/ajakan. Tidak menyentuh endpoint/Resource/logika kuota. — `resources/js/components/open/IsturaOpenPromo.tsx` — (test: `npm run build` pass; diagnostics clean)

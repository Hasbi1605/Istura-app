...
- `ScheduleService.php` — perhitungan horizon/slot, status default runtime, `buildHorizon()`, kapasitas peserta per slot, serta validasi atomik slot publik H/H+1 yang hanya boleh terbuka lewat marker eksplisit `schedule_overrides.public_early_opened_at`.

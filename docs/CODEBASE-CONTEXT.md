...
- `ScheduleService.php` — perhitungan horizon/slot, status default runtime berdasarkan `OperationalSchedulePolicy`, `buildHorizon()`, kapasitas peserta per slot, serta validasi atomik slot publik H/H+1 yang hanya boleh terbuka lewat marker eksplisit `schedule_overrides.public_early_opened_at`. **Tanggal merah nasional (`national_holidays`) selalu `Closed` dan tidak bisa dibuka override admin (buka rentang maupun slot).** Guard ini berlaku di `buildHorizon()`/`resolveSlotStatus()` (kalender) dan `slotStatusFor()`/`slotStatusesFor()` (validasi booking), sehingga override `Available` tidak pernah membuka libur nasional; booking aktif yang sudah ada tetap ditampilkan karena dicek lebih dulu.
...
### 2.6 Database
- `database/migrations` (~45 file). Tabel inti: users, bookings, booking_sequences, booking_slots, booking_slot_locks, schedule_overrides, feedbacks, faqs, wa_templates, footer_contacts, site_settings, audit_logs, trusted_devices, national_holidays + tabel sistem (sessions, cache, jobs, personal_access_tokens). `site_settings` juga menyimpan `schedule_policy` untuk hari operasional default yang bisa diubah admin tanpa deploy kode. `bookings.source`/`created_by_admin_id` membedakan booking publik dan buatan admin. Kolom legacy `schedule_overrides.short_notice_*` tetap ada untuk kompatibilitas data lama, tetapi flow aktif jadwal publik H/H+1 tidak lagi memakai `custom=true` sebagai sinyal publik; slot H/H+1 hanya tampil/valid publik bila override `Available` memiliki `public_early_opened_at` (diisi saat admin membuka slot yang sedang berada di H/H+1 dan jamnya belum lewat). `feedbacks` menyimpan dua rating tambahan (`guide_quality`, `facility_comfort`) dan profil kunjungan (`visited_before`, `discovery_source`, `discovery_source_other`); semuanya nullable agar data lama tetap kompatibel. Relasi feedback 1:N (hasMany): `booking_id` non-unique, kuota = `bookings.group_size`, token expired via `bookings.feedback_expires_at`.

...
> `IsturaOpen.md` (modul Istura Open), dan `AGENTS.md` (aturan kerja).
>
...
- `ScheduleService.php` — perhitungan horizon/slot, status default runtime berdasarkan `OperationalSchedulePolicy`, `buildHorizon()`, kapasitas peserta per slot, serta validasi atomik slot publik H..H+2 (minimal H+3) yang hanya boleh terbuka lewat marker eksplisit `schedule_overrides.public_early_opened_at` (`PUBLIC_MIN_LEAD_DAYS=3`, `PUBLIC_EARLY_MAX_DAYS=2`). **Tanggal merah nasional (`national_holidays`) selalu `Closed` dan tidak bisa dibuka override admin (buka rentang maupun slot).** Guard ini berlaku di `buildHorizon()`/`resolveSlotStatus()` (kalender) dan `slotStatusFor()`/`slotStatusesFor()` (validasi booking), sehingga override `Available` tidak pernah membuka libur nasional; booking aktif yang sudah ada tetap ditampilkan karena dicek lebih dulu.
- `OperationalSchedulePolicy.php` — policy hari operasional default admin (`site_settings.schedule_policy`), default permanen Senin-Jumat buka + Sabtu/Minggu tutup, dipakai `ScheduleService` untuk menentukan `operational_closed`.
...
- **Policies** (`app/Policies`): `BookingPolicy`, `FeedbackPolicy`, `OpenEventPolicy`, `ScheduleOverridePolicy`.
- **Rules** (`app/Rules`): `SafePublicUrl`, `VisitTime`.
- **Support** (`app/Support`): `PublicCache` (CMS/schedule cache, plus HTML homepage origin 5 menit; browser/edge wajib revalidate; invalidated tiap CMS save dan tiap deploy), `SiteContentDefaults`,
  `SeoMeta` (canonical URL, metadata, JSON-LD, sitemap, robots). Homepage `/` dikecualikan dari session, CSRF, dan shared session-error middleware supaya tidak mengirim cookie dan aman dicache. Header `s-maxage=0` sengaja mencegah edge menahan markup Vite lama karena deploy saat ini menghapus hash asset lama; mutasi SPA tetap menyiapkan cookie Sanctum saat aksi pertama. `SiteContentDefaults` adalah fallback utama untuk konten landing, widget WhatsApp, banner Istura Open, serta copy wizard publik.
...
### 2.5 Routes (`routes/`)
- **api.php** — 3 grup: `public/*` (rate-limited; termasuk `open-event` + `open-registrations/*` throttle `public-open`; feedback dipisah menjadi `public-feedback-view` dan `public-feedback-submit` agar rombongan satu IP tidak mudah kena limiter lama), `auth/*` (login throttle + sanctum), `admin/*` (middleware `admin-access`; READ routes terbuka untuk viewer, MUTATION routes dibungkus middleware `operator` + `throttle:admin-mutations` 60/menit per akun, nested `super-admin` untuk users; endpoint baca/dashboard/export/audit tidak ikut limiter mutasi; detail feedback memakai id submission karena kode booking tidak unik pada multi-feedback). `cms/site-content` menerima PUT JSON dan POST multipart untuk upload aset gambar landing.
- **web.php** — `/robots.txt`, `/sitemap.xml`, `/info/alur-kunjungan` (OG tags WA preview), `/` (SPA shell stateless dengan HTML cache origin 5 menit; `Cache-Control: public, max-age=0, s-maxage=0, must-revalidate`), lalu catch-all `/{any}` → `view('app')` (SPA dengan metadata/JSON-LD dan konten ringkasan server-rendered untuk crawler). Root homepage memakai preview sosial khusus `/assets/istura-home-preview.jpg`, sedangkan `/info/alur-kunjungan` tetap memakai preview peraturan/alur untuk link pembooking yang disetujui. Regex catch-all mengecualikan `api`.
...
### 2.6 Database
- `database/migrations` (~45 file). Tabel inti: users, bookings, booking_sequences, booking_slots, booking_slot_locks, schedule_overrides, feedbacks, faqs, wa_templates, footer_contacts, site_settings, audit_logs, trusted_devices, national_holidays + tabel sistem (sessions, cache, jobs, personal_access_tokens). `site_settings` juga menyimpan `schedule_policy` untuk hari operasional default yang bisa diubah admin tanpa deploy kode. `bookings.source`/`created_by_admin_id` membedakan booking publik dan buatan admin. Kolom legacy `schedule_overrides.short_notice_*` tetap ada untuk kompatibilitas data lama, tetapi flow aktif jadwal publik H..H+2 tidak lagi memakai `custom=true` sebagai sinyal publik; slot H..H+2 hanya tampil/valid publik bila override `Available` memiliki `public_early_opened_at` (diisi saat admin membuka slot yang sedang berada di H..H+2 dan jamnya belum lewat). `feedbacks` menyimpan dua rating tambahan (`guide_quality`, `facility_comfort`) dan profil kunjungan (`visited_before`, `discovery_source`, `discovery_source_other`); semuanya nullable agar data lama tetap kompatibel. Relasi feedback 1:N (hasMany): `booking_id` non-unique, kuota = `bookings.group_size`, token expired via `bookings.feedback_expires_at`.
...
- **admin/**: `AdminApp.tsx`, `AdminErrorBoundary.tsx` (fallback render panel dengan aksi muat ulang/kembali ke publik), `AdminShell.tsx`, `AdminDashboard.tsx`, `AdminCmsManagers.tsx` (preview + upload langsung logo navbar/footer, background CTA, dan foto panel aktivitas; semua aset ikut satu draft/save Landing Page. Draft Landing Page yang belum disimpan dilindungi saat pindah menu, muat ulang, logout, kembali ke publik, atau menutup tab. Tab "Wizard Publik" mengedit copy fixed-flow untuk `BookingWizard` dan `FeedbackScreen` tanpa mengubah step/validasi/ikon/gambar), `AdminFeedbackList.tsx`, `AdminSystemPages.tsx`, `BookingScreen.tsx` (Atur Kloter dengan total terkunci/koreksi eksplisit dan checkbox konfirmasi untuk risiko, Pindah Jadwal Langsung, Buat Booking Manual dengan mode kloter otomatis/manual, surat opsional, tanpa textarea catatan internal, dan modal hard delete booking dengan konfirmasi ketik kode; error modal direset per konteks dan warning identitas dibedakan dari notifikasi sukses), `ScheduleManager.tsx` (toggle slot/range jadwal, pengaturan pola operasional default via `GET/PUT /api/admin/schedule/policy`, termasuk H/H+1 yang default tertutup dan dapat dibuka dengan kontrol jadwal biasa; slot terisi hanya menampilkan ringkasan dan tombol Buka di Booking; sumber waktu diperbarui per menit agar batas hari/bulan tidak stale), `IsturaOpenManager.tsx` (3 tab: Pengaturan & Hari, Pendaftar, dan Feedback; modal Buat/Edit event memakai `OpenEventDateCalendar.tsx` — kalender multi-select selaras admin Jadwal, klik langsung toggle tanggal + mode rentang dua-klik; toolbar event punya Edit event untuk nama/tanggal/kuota/add-on/teks persetujuan, serta aksi aktif/arsip/hapus draft; dialog konflik menyediakan tombol langsung ke detail Booking), `ExportModals.tsx`, `WeeklyPosterModal.tsx`, `TwoFactorChallenge.tsx`, `TwoFactorSetup.tsx`.
- **booking/**: `BookingWizard.tsx` (wizard 8 langkah; teks step, helper, MIKY, label form, upload, persetujuan, sukses, dan tombol berasal dari `siteContent.bookingWizard` dengan fallback default. Rombongan >80 mendapat rincian kloter + CTA diskusi awal di Data Instansi; setelah memilih jam, callout penyesuaian + CTA yang sama tampil sebagai card horizontal di bawah kalender dan daftar jam, dengan pesan terisi instansi, jumlah peserta, tanggal, dan jadwal per kloter). **feedback/**: `FeedbackScreen.tsx` (wizard 4 langkah: penilaian inti → tentang kunjungan → detail pengalaman → komentar; copy, label rating, label sumber informasi, dan opsi chip berasal dari `siteContent.feedbackWizard`). **home/**: `HomeScreen.tsx` (video YouTube berupa facade lokal lalu iframe dibuat setelah klik; hero merender satu pose MIKY aktif dengan kandidat `srcset` 640w/1024w dan area `2 / 3` terkunci sebelum gambar tiba. Pose awal dipreload lewat Blade; sesudah setiap pose tampil, browser fetch+decode satu pose berikutnya pada prioritas rendah agar copy dan pose tetap berganti langsung tanpa memuat semua pose saat start. Gambar bawah halaman memakai `loading="lazy"` tanpa warm-up masal yang dapat berebut bandwidth hero; panel "Aktivitas di Istana" memakai `HorizontalAccordion`: di desktop melebar horizontal saat hover/focus; di mobile (≤980px) menjadi tap-to-expand vertikal — panel aktif membuka tinggi besar beranimasi + zoom Ken Burns, panel lain menyusut jadi bar judul, hormati `prefers-reduced-motion`). **open/**: `IsturaOpenWizard.tsx` (wizard publik 5 langkah dalam shell/action bar yang sama dengan wizard rombongan, tetapi panel konteks event menggantikan MIKY; pilih hari → data diri → add-on → tinjau → sukses + tombol grup WA; ada lookup/self-cancel via NIK+WhatsApp dan pilihan hari direset otomatis bila realtime menutup/menghabiskan kuota), `IsturaOpenPromo.tsx` (popup sekali per event per pengunjung + banner persisten).
...
### 5.2 Flow Admin
- **Preferensi tampilan:** tombol `Aa` melayang di kanan bawah membuka pilihan Ringkas/Nyaman/Besar. Skala font, kontrol, sidebar, topbar, dan ruang konten hanya diterapkan selama `AdminShell` aktif; preferensi disimpan per browser lewat `localStorage` dan dibersihkan dari root saat kembali ke sisi publik.
- **Override Jadwalkan Ulang:** admin dapat mengusulkan jadwal pada hari yang sama dan, bila slot tujuan sudah terisi, wajib mengaktifkan konfirmasi `allowOverbook`; tanggal lampau dan jam yang sudah lewat tetap ditolak.

...
- **Reload global admin:** tombol "Muat ulang" dipindah ke top bar `AdminShell` (kiri blok akun). Menekan tombol memanggil `useIsturaData.reloadAdmin()` (re-fetch bookings/feedbacks/schedule via nonce + `refetchOpenEvent`) **dan** me-remount layar aktif via `key` di `AdminApp` (untuk data milik komponen: Istura Open, Dashboard, Users, Audit). Feedback memakai toast `admin-schedule-toast` ("Memuat ulang…" → "Data diperbarui"); `adminRefreshing` dari hook menyetir state busy. Tombol "Muat ulang" lokal di `IsturaOpenManager` dihapus (digantikan yang global). Catatan: remount mereset state dalam layar (filter/drawer/draft editor belum tersimpan).

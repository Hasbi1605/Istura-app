...
- **Booking (8 langkah):** Selamat Datang → Contact Person (precheck NIK/WA) → Instansi (rincian pembagian bila >80) → Pilih Jadwal (callout + CTA WhatsApp penyesuaian kloter setelah jam dipilih) → Upload Surat (≤5MB) → Review → Pernyataan → Selesai (kode `ISTURA-YYYY-NNNN`, status Pending). Submit mengunci slot via transaksi DB + broadcast realtime ke admin; diskusi WhatsApp tidak mengubah slot sebelum admin melakukan override manual.
